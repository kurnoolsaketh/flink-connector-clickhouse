...
1. In a new branch, update `version.txt` to the new version number.
2. Create a PR to main, merge the version update commit.
3. Tag the version update commit as `v<version_number>` (e.g. v0.1.3 for version 0.1.3) and push the tag to origin. **This will automatically trigger a Github action that will PR an update to all version references in the project and create a draft Github release for the new version.**
```bash
git tag v0.1.3 <update_commit_hash>
...
Run the `Build and Publish to Maven Central` Github action by providing the target release version.
